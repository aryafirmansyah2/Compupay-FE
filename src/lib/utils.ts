...
export function formatFileSize(bytes: number): string {
  if (bytes === 0) return "0 Bytes"; // Jika ukuran file adalah 0 bytes, tampilkan "0 Bytes"
  const k = 1024; // Pembagi untuk konversi
  const sizes = ["Bytes", "KB", "MB", "GB", "TB", "PB"]; // Daftar satuan ukuran file
  const i = Math.floor(Math.log(bytes) / Math.log(k)); // Menentukan indeks untuk ukuran file yang sesuai
  return parseFloat((bytes / Math.pow(k, i)).toFixed(1)) + " " + sizes[i]; // Mengembalikan ukuran file dalam format yang sesuai
}
